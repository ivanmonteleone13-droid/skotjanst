    ...
    <section className="bg-[#3D2914] py-20 text-white">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-4 text-amber-100/80">Ordagrant utdrag från verifierade kundomdömen på Reco.se.</p>
        </div>
        <div className="mt-12 grid gap-6 md:grid-cols-2">
          {business.testimonials.map((review, index) => (
            <figure key={index} className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur">
              ...
              <blockquote className="text-sm leading-relaxed text-amber-50/90">&ldquo;{review.text}&rdquo;</blockquote>
              <figcaption className="mt-4 text-xs text-amber-100/60">
                — {review.author} ·{" "}
                <a href={review.sourceUrl} target="_blank" rel="noopener noreferrer" className="underline hover:text-amber-100">
                  {review.source}
                </a>
